...
function ContasPagarContent() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      setCategorias(catRes.data.filter((c) => c.tipo === 'pagar' || c.tipo === 'ambos'));
  ...
  const handleBaixa = async () => {
    ...
    try {
      ...
      showSnack('Baixa registrada com sucesso.');
      setBaixaOpen(false);
      fetchAll();
    } catch (e) {
      showSnack(extractApiErrorMessage(e, 'Erro ao dar baixa.'), 'error');
    } finally {
  ...
  const barData = [
    { name: 'Pendente', valor: resumo?.total_pagar_pendente ?? 0, fill: '#f59e0b' },
    { name: 'Vencido',  valor: resumo?.total_pagar_vencido  ?? 0, fill: '#ef4444' },
    { name: 'Pago/mês', valor: resumo?.total_pagar_pago_mes ?? 0, fill: '#22c55e' },
  ];

  // Monthly trend from contas list (group by month)
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <PageHeader
        ...
        subtitle="Controle de despesas, vencimentos e baixas"
        actions={
          <>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        {[
          { label: 'A Pagar',      value: loading ? '—' : fmtBRL(resumo?.total_pagar_pendente ?? 0), icon: <TrendingDownIcon />, color: '#f59e0b', subtitle: 'pendente' },
          { label: 'Vencido',      value: loading ? '—' : fmtBRL(resumo?.total_pagar_vencido  ?? 0), icon: <WarningIcon />,      color: '#ef4444', subtitle: 'em atraso' },
          { label: 'Pago este mês',value: loading ? '—' : fmtBRL(resumo?.total_pagar_pago_mes ?? 0), icon: <CheckCircleIcon />,  color: '#22c55e', subtitle: 'mês corrente' },
        ].map((kpi) => (
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        ...
        <Grid item xs={12} md={7}>
          <Paper sx={{ p: 2.5 }}>
            <Typography variant="subtitle2" fontWeight={700} sx={{ mb: 2 }}>
              Tendência — Valor a Pagar por Mês
            </Typography>
            {loading ? (
              <Skeleton variant="rectangular" height={180} sx={{ borderRadius: 2 }} />
            ) : areaData.length === 0 ? (
      ...
      >
        ...
        <TextField
          ...
          InputLabelProps={{ shrink: true }}
          helperText="Opcional — período a que a despesa se refere (pode diferir do vencimento)"
          fullWidth
        />
