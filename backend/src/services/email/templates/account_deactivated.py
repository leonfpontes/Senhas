"""Account/tenant deactivation (self-service, reversible) email template."""
...
def render_account_deactivated_email(user_name: str, reactivation_url: str) -> str:
    """Generate inline-CSS email confirming a reversible account deactivation.

    Args:
        user_name: Display name of the user (escaped before rendering).
        reactivation_url: Full URL to the reactivation page.

    Returns:
        HTML string ready to send via email provider.
    """
    name = _esc(user_name) or "usuário"
    safe_url = escape(reactivation_url)
    ...
    <table role="presentation" width="560" cellpadding="0" cellspacing="0"
      ...
      <tr>
        <td style="background:linear-gradient(135deg,#6366f1 0%,#ec4899 100%);padding:36px 40px;text-align:center;">
          ...
          <p style="margin:8px 0 0;color:rgba(255,255,255,0.85);font-size:14px;">Conta desativada</p>
        </td>
      </tr>
      ...
      <tr>
        <td style="padding:40px 40px 24px;">
          ...
          <p style="margin:0 0 24px;color:#374151;font-size:15px;line-height:1.6;">
            Confirmamos a desativação da sua conta e do seu terreiro no GiraHub, conforme
            solicitado. Isso é diferente de uma exclusão permanente:
            <strong>todos os seus dados continuam guardados</strong> (giras, tickets, médiuns,
            associados) e nenhuma cobrança futura será feita.
          </p>
            ...
            <tr>
              <td style="background:#EFF6FF;border-left:4px solid #3B82F6;border-radius:4px;padding:12px 16px;">
                <p style="margin:0;color:#1E40AF;font-size:13px;line-height:1.5;">
                  Você pode voltar a usar o GiraHub quando quiser — sua conta reativa no
                  plano gratuito, com todos os dados anteriores preservados.
                </p>
              </td>
            </tr>
          </table>
          ...
          <p style="margin:24px 0 0;color:#6B7280;font-size:13px;line-height:1.6;">
            Se você não solicitou essa desativação, entre em contato imediatamente com
