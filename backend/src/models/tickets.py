...
class Ticket(SoftDeleteModel):
    ...
    emitido_por_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
    
    numero: Mapped[int] = mapped_column(Integer, nullable=False)  # Sequential ticket number
    status: Mapped[TicketStatus] = mapped_column(SQLEnum(TicketStatus), default=TicketStatus.EMITTED, nullable=False)
